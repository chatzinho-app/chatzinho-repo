...
const buttonVariants = cva(
  'inline-flex items-center justify-center description-header transition-colors focus:outline-none disabled:cursor-not-allowed ',
  {
    variants: {
      ...
      size: {
        default: 'py-2 px-5 rounded-full',
      },
    },
    defaultVariants: {
      variant: 'default',
...
export default function Button({ size, variant, loading, ...props }: ButtonProps) {
  const className = twMerge(props?.className, loading ? "disabled:cursor-progress" : "")

  return <button {...props} className={cn(buttonVariants({ size, variant, className }))} disabled={props?.disabled || loading}>
    {props?.children}
    {loading && (
